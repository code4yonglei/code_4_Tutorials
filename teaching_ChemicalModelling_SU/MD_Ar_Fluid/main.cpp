...
int Natoms;
double *m = NULL;
double R=0.0, Rcut=2.5, R2cut=0.0, RIcut=0.0, RI2cut=0.0, RI6cut=0.0, RI12cut=0.0;
	...
	for(int k=0; k<=50000; k++) { //Loop over timesteps 
		...
		if(k % 100 == 0) {
			std::cout << std::setw(8) << k << std::setw(12) << elapsed_time
				<< std::setw(15) << T << std::setw(15) << P << std::setw(12) << U 
				<< std::setw(12) << KE << std::setw(12) << TE 
...
void init(int Natoms, const char* filename){
	...
	m     = new double[Natoms];  // Memory Management
	r     = new double*[Natoms];
	v     = new double*[Natoms];
	f     = new double*[Natoms];
	...
	for(i=0; i<Natoms; i++){ // intialize mass, velocities, forces
		m[i] = 1.0;
...
void calc_virial_force(void){
	...
	for(i=0; i<Natoms; i++){ // forces acting on specific atoms
		f[i][0] = 0.0;
		f[i][1] = 0.0;
		f[i][2] = 0.0;
	}
...
void dump_stats(std::ofstream & enerFile, int config){
	enerFile << std::setw(8) << config << std::setw(12) << elapsed_time
		<< std::setw(12) << T  << std::setw(12) << P << std::setw(12) << U
		<< std::setw(12) << KE << std::setw(12) << TE 
		<< std::setw(15) << px << std::setw(15) << py << std::setw(15) << pz
		<< std::endl;
}
